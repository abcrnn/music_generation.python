...
def note_to_string(note, duration, default_len, key_accidentals, cur_accidentals):
    ...
    #modified by @Dat
    #need for translate: https://www.tutorialspoint.com/python/string_translate.htm
    # trantab = str.maketrans(None, '_=^')
    notes = []
    for n in scale:
        # notes.append(n.lower().translate(trantab))
        notes.append(n.lower().replace('_','').replace('=','').replace('^',''))


    notes = [n.lower().translate(str.maketrans('','','_=^'))
             for n in scale]   # for semitone: the name of the note
    #convert to compatible with p
    ...
    while notes:
        ...
        # else handle notes one by one or as a chord
            ...
            if broken_rythm_symbol:
                output.write(ensureUtf(broken_rythm_symbol))
